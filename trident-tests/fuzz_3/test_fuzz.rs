...
/// Main fuzz test structure for mixed spot + conditional trading operations.
...
impl FuzzTest {
    ...
    fn start(&mut self) {
        ...
        let (dao, multisig) = methods::futarchy::initialize_dao(
            ...
            self.quote_usdc,
            init_dao_params,
            Some("Initialize DAO for Mixed Trading Fuzz"),
        );
        self.dao = dao;
    ...
    fn flow_spot_swap_alice(&mut self) {
        self.spot_swap_flow(self.alice.pubkey(), "Spot Swap - Alice");
    }

    #[flow]
    fn flow_spot_swap_bob(&mut self) {
        self.spot_swap_flow(self.bob.pubkey(), "Spot Swap - Bob");
    }

    #[end]
    fn end(&mut self) {
        self.assert_global_invariants();
        self.finalize_proposal_and_assert_spot_alignment();
    }
}
