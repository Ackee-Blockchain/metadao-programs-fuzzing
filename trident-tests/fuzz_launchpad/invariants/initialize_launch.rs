...
impl FuzzTest {
    pub fn verify_initialize_launch_invariants(
        ...
        assert_eq!(launch, expected_launch_pda, "Launch PDA must match seeds");
        assert_eq!(
            post_launch.pdaBump, expected_launch_bump,
        ...
        assert_eq!(
            launch_signer, expected_signer_pda,
            "launch_signer PDA must match seeds"
        );
        assert_eq!(
            post_launch.launchSigner, launch_signer,
            "Launch.launchSigner must equal the passed launch_signer PDA"
        );
        assert_eq!(
            post_launch.launchSignerPdaBump, expected_signer_bump,
        ...
        assert_eq!(
            quote_vault, expected_quote_vault,
            "quote_vault must be the ATA(quote_mint, launch_signer)"
        );
        assert_eq!(
            base_vault, expected_base_vault,
        ...
        assert_eq!(
            post_launch.minimumRaiseAmount, args.minimumRaiseAmount,
            "minimumRaiseAmount must match args"
        );
        assert_eq!(
            post_launch.monthlySpendingLimitAmount, args.monthlySpendingLimitAmount,
            "monthlySpendingLimitAmount must match args"
        );
        assert_eq!(
            post_launch.monthlySpendingLimitMembers, args.monthlySpendingLimitMembers,
            "monthlySpendingLimitMembers must match args"
        );
        assert_eq!(
            post_launch.launchAuthority, launch_authority,
            "launchAuthority must match passed launch_authority"
        );
        assert_eq!(
            post_launch.launchQuoteVault, quote_vault,
            "launchQuoteVault must match passed quote_vault"
        );
        assert_eq!(
            post_launch.launchBaseVault, base_vault,
            "launchBaseVault must match passed base_vault"
        );
        assert_eq!(
            post_launch.baseMint, base_mint,
            "baseMint must match passed base_mint"
        );
        assert_eq!(
            post_launch.quoteMint, quote_mint,
            "quoteMint must match passed quote_mint"
        );
        assert_eq!(
            post_launch.secondsForLaunch, args.secondsForLaunch,
        ...
        assert_eq!(post_launch.seqNum, 0, "seqNum must start at 0");
        assert_eq!(
            post_launch.totalCommittedAmount, 0,
            "totalCommittedAmount must start at 0"
        );
        assert_eq!(
            post_launch.totalApprovedAmount, 0,
            "totalApprovedAmount must start at 0"
        );
        assert_eq!(
            post_launch.unixTimestampStarted, None,
            "unixTimestampStarted must start as None"
        );
        assert_eq!(
            post_launch.unixTimestampClosed, None,
            "unixTimestampClosed must start as None"
        );
        assert_eq!(post_launch.dao, None, "dao must start as None");
        assert_eq!(post_launch.daoVault, None, "daoVault must start as None");
        assert_eq!(
            post_launch.performancePackageGrantee, args.performancePackageGrantee,
            "performancePackageGrantee must match args"
        );
        assert_eq!(
            post_launch.performancePackageTokenAmount, args.performancePackageTokenAmount,
            "performancePackageTokenAmount must match args"
        );
        assert_eq!(
            post_launch.monthsUntilInsidersCanUnlock, args.monthsUntilInsidersCanUnlock,
            "monthsUntilInsidersCanUnlock must match args"
        );
        assert_eq!(
            post_launch.teamAddress, args.teamAddress,
            "teamAddress must match args"
        );
        assert_eq!(
            post_launch.additionalTokensAmount, args.additionalTokensAmount,
        ...
        if args.additionalTokensAmount > 0 {
            ...
            assert_eq!(
                post_launch.additionalTokensRecipient, None,
                "additionalTokensRecipient must be None when additionalTokensAmount == 0"
            );
        }
        assert!(
            post_launch.additionalTokensClaimed == false,
            "additionalTokensClaimed must start false"
        );
        assert_eq!(
            post_launch.unixTimestampCompleted, None,
            "unixTimestampCompleted must start as None"
        );
        assert!(
            post_launch.isPerformancePackageInitialized == false,
        ...
        let post_quote_vault_amount = self
            ...
            .amount;
        assert_eq!(
            post_quote_vault_amount, pre_quote_vault_amount,
        ...
        let post_base_vault = self
            ...
            .account;
        assert_eq!(
            post_base_vault.mint, base_mint,
            "base_vault mint must equal base_mint"
        );
        assert_eq!(
            post_base_vault.owner, launch_signer,
