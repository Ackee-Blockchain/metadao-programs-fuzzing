use crate::constants::{FUNDING_RECORD_SEED_PREFIX, TOKEN_PROGRAM_ID};
...
impl FuzzTest {
    pub fn verify_claim_invariants(
        ...
        assert!(
            pre_funding_record.isTokensClaimed == false,
        ...
        assert!(
            post_funding_record.isTokensClaimed == true,
        ...
        assert_eq!(
            vault_delta, expected_token_amount,
            "vault must decrease by expected_token_amount"
        );
        assert_eq!(
            funder_delta, expected_token_amount,
