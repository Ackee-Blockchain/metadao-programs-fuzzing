...
build-bid-wall:
	cargo build-sbf --manifest-path programs/bid_wall/Cargo.toml --arch sbfv1

build-futarchy:
	cargo build-sbf --manifest-path programs/futarchy/Cargo.toml --arch sbfv1

build-price-based-performance-package:
	cargo build-sbf --manifest-path programs/price_based_performance_package/Cargo.toml --arch sbfv1

build-v07-launchpad:
	cargo build-sbf --manifest-path programs/v07_launchpad/Cargo.toml --arch sbfv1

build-conditional-vault:
	cargo build-sbf --manifest-path programs/conditional_vault/Cargo.toml --arch sbfv1
